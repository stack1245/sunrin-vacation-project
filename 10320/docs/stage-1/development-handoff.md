...
| TypeScript 단위 테스트 | 완료 | 현재 재검증 기준 16개 통과 |
...
- 중앙 복도와 연구 자료실·화학 실험실·보안 통제실 이동을 구현한다.
- 각 성공 이벤트 뒤 새 불변 상태를 만들고 `save()`한다.
- 네트워크 실패 경고와 재시도 큐를 만든다.

### 3순위: 퍼즐과 최종 클리어

- 자료실 단서, 화학 퍼즐, 보안 통제실, 기밀 문서 보관실 퍼즐을 순서대로 연결한다.
- 기밀 문서 획득 후 외부 탈출을 구현한다.
