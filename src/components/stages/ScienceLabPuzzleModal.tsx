...
export function ScienceLabPuzzleModal() {
  ...
  return (
    <div
      className="fixed inset-0 z-[60] flex items-center justify-center bg-[#050b10]/92 p-3 backdrop-blur-md sm:p-4"
      role="dialog"
      aria-modal="true"
      aria-labelledby="science-lab-puzzle-title"
    >
      <div className="game-interface max-h-[calc(100dvh-1.5rem)] w-full max-w-2xl overflow-y-auto rounded-[4px] border border-[var(--game-border-strong)] bg-[var(--game-surface)] shadow-2xl shadow-black/60 sm:max-h-[90vh]">
        <header className="border-b border-[var(--game-border)] bg-[var(--game-surface-raised)] px-5 py-4 sm:px-7">
          <div className="flex items-start justify-between gap-4">
            ...
            <button
              ...
              className="facility-focus min-h-10 shrink-0 rounded-[3px] border border-[var(--game-border)] bg-[var(--game-void)] px-3 py-2 font-mono text-xs text-[var(--game-muted)] transition-colors hover:border-[var(--game-border-strong)] hover:text-[var(--game-text)]"
            >
              닫기 (ESC)
            </button>
          </div>
        ...
        <div className="space-y-5 px-5 py-6 sm:px-7">
            ...
            {definition.choices.map((choice, index) => {
              ...
              return (
                <button
                  ...
                  className={
                    selected
                      ? "facility-focus min-h-24 rounded-[3px] border border-[var(--game-accent)] bg-[#14261f] p-4 text-left shadow-lg shadow-black/30 sm:min-h-28"
                      : "facility-focus min-h-24 rounded-[3px] border border-[var(--game-border)] bg-[var(--game-void)] p-4 text-left transition-colors hover:border-[var(--game-border-strong)] hover:bg-[var(--game-surface-raised)] sm:min-h-28"
                  }
                >
          ...
          <div className="flex flex-col-reverse gap-2 border-t border-[var(--game-border)] pt-5 sm:flex-row sm:justify-end">
            <button
              type="button"
              onClick={cancelActiveRequest}
              className="facility-button facility-focus px-5"
            >
              취소
            </button>
            <button
              ...
              className="facility-button-primary facility-focus px-6"
            >
              설정 적용
            </button>
          </div>
