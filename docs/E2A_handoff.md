...
`develop/stage-1`에는 B의 외부·입구·중앙 복도와 E의 보안 통제실이 연결됐다. 다만 C 연구 자료실과
D 과학 실험실이 아직 참조 Room이어서 `archiveClueFound`, `scienceLabPuzzleSolved`를 정상 플레이로
얻을 수 없다. 따라서 통제실까지 이어지는 전체 진행 QA는 C·D 통합 후 수행한다.

```text
연구소 외부 ──> 연구소 입구 ──> 중앙 복도 ──> 연구 자료실 ──> 과학 실험실 ──> 보안 통제실
  B 실제          B 실제         B 실제          C 참조          D 참조          E 실제
  └────────────── 정상 진행 가능 ──────────────┘   ↑ C·D 통합 후 전체 QA
```
...
| 검사 | 명령 | 결과 |
| --- | --- | --- |
| 단위 테스트 | `npm test` | **129 / 129 통과** (B 파트와 A 통합 테스트 포함) |
