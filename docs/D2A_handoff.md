...
- D 파트 승인 코드 `SEC-8042-CTRL`을 E 파트의 세션별 동적 OTP와 분리했다. `420042`는 E 파트 기준 설정의 테스트 값일 뿐 실제 게임에서는 단말을 열 때마다 새 OTP가 발급된다. 현재 E 파트의 기능 해금 계약은 코드 문자열이 아니라 `scienceLabPuzzleSolved` 플래그를 사용한다.
