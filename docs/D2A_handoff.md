...
- Layer7 부스 프로젝트를 시각 참고점으로 삼아 메인·인증·스테이지 선택·공통 HUD·퍼즐 모달·전체 Phaser Room과 문서 보관실 퍼즐을 하나의 산업 시설 팔레트로 통일했다.
- `FacilityShell`, `facility-*`, `--game-*` 공통 계약을 추가하고 정상·경고·잠김 상태가 색상과 문구로 함께 구분되도록 정리했다.
...
- 기능 구현 기준 미완료 항목 없음.
- 최종 일러스트·사운드·실험실 전용 원화 자산은 범위에 포함하지 않았고 Phaser 기본 도형과 텍스트로 구성했다.
...
| 브라우저 QA | 메인·로그인·공통 HUD·중앙 복도·과학 실험실·문서 보관실 | 개발 서버 오류 없이 렌더링되고 공통 팔레트·상태 대비·캔버스 배치를 확인. 인증된 실제 D 플레이는 미실행 |
